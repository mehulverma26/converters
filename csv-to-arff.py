...
#
# Python 3.6 was used to make this
#
# @Description
...
fileToRead = "data.csv"  # csv file name or absolute path to be open.
fileToWrite = "data.arff"  # name as how you'll save your arff file.
relation = "spotify"  # how you'll like to call your relation as.
...
for j in range(0, totalCols):
    for i in range(1, totalRows):
        ...
        try:
            if allData[i][j] == str(float(allData[i][j])) or allData[i][j] == str(
                int(allData[i][j])
            ):
                print
...
for j in range(1, totalRows):
    for i in range(0, totalCols):
        try:
            if allData[j][i] == str(float(allData[j][i])) or allData[j][i] == str(
                int(allData[j][i])
            ):
